...
| Entry: two taps | ✅ Each slot has a rail of "from"s — the four disconnected shapes, then every connection — and under it what that "from" can go to, grouped by rung: Hold, connections, then finishes by kind. Tap from, tap to, and the line is written for you — `Front headlock => Darce x25` — the first tap creating the list, later taps adding to it. A target key (none / x25 / x50) sticks between taps; tokui defaults to x25, kaizen to x50. A destination that isn't on the ladder is typed in a box that suggests as you go. Chips already on the list go dark. Empty slots also offer the coach's sets, trimmed to the cap. |
